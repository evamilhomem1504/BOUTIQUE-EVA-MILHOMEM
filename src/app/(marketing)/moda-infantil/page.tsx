...
import { getInfantilMenino, getInfantilMenina } from "@/lib/data";
import { siteConfig } from "@/config/site";

export const metadata: Metadata = {
  title: "Moda Infantil",
  description: "Roupas infantis premium para meninas e meninos — Boutique Eva Milhomem.",
};

export default function ModaInfantilPage() {
  const menino = getInfantilMenino();
  const menina = getInfantilMenina();
  ...
  return (
    <>
      ...
      <section className="relative bg-noir pt-28 pb-16 md:pt-36 md:pb-20 overflow-hidden">
        ...
        <div className="container-boutique relative z-10">
          ...
          <h1 className="font-heading font-light text-5xl md:text-7xl text-off-white leading-tight mb-4">
            Moda <span className="italic text-gradient-gold">Infantil</span>
          </h1>
          <p className="text-off-white/50 font-sans text-base max-w-md">
            Peças premium para os pequenos com o estilo e a qualidade que a Boutique Eva Milhomem oferece.
          </p>
        </div>
        ...
        <div className="container-boutique py-4">
          <div className="flex items-center gap-8 overflow-x-auto scrollbar-none">
            {[
              ...
              { label: "Menino", href: "#menino" },
              { label: "Menina", href: "#menina" },
            ].map((f, i) => (
              <a
                key={f.label}
                href={f.href}
            ...
            <div className="ml-auto shrink-0 hidden md:block">
              <a
                href={`https://wa.me/${siteConfig.whatsappNumber}?text=${encodeURIComponent("Olá! Gostaria de ver as peças infantis disponíveis.")}`}
                target="_blank"
      ...
      {/* ── MENINO ───────────────────────────────────────────────── */}
      {menino.length > 0 && (
        <section id="menino" className="py-16 md:py-24 bg-off-white">
          <div className="container-boutique">
            <AnimatedSection className="mb-12">
              <div className="flex items-center gap-4 mb-2">
                <div className="w-1 h-8 bg-noir" />
                <SectionHeader
                  eyebrow="Infantil"
                  title="Menino"
                  centered={false}
      ...
      {/* ── MENINA ───────────────────────────────────────────────── */}
      {menina.length > 0 && (
        <section id="menina" className="py-16 md:py-24" style={{ background: "#ede8e2" }}>
          <div className="container-boutique">
            <AnimatedSection className="mb-12">
              <div className="flex items-center gap-4 mb-2">
      ...
      {/* Sem produtos */}
      {menino.length === 0 && menina.length === 0 && (
        ...
        <div className="container-boutique text-center">
          <AnimatedSection>
            ...
            <h2 className="font-heading font-light text-4xl md:text-5xl text-off-white mb-4">
              Quer ver mais tamanhos?
            </h2>
            <p className="text-white/40 font-sans text-base max-w-xs mx-auto mb-8">
              Temos mais opções disponíveis na loja. Fale com a gente.
            </p>
            <a
              href={`https://wa.me/${siteConfig.whatsappNumber}?text=${encodeURIComponent("Olá! Gostaria de ver as peças infantis disponíveis.")}`}
              target="_blank"
